...
def pinecone_document_noexists(
    file_name, reporting_period, product_name, site_name, index_name
):
    # initialize pinecone
    # pinecone.init(
        ...
        for page_num, page in enumerate(pdf.pages):
            ...
            if tables:
                ...
                for table_index, table in enumerate(tables):
                    ...
                    table_filename = (
                        f"{sfilename}_table_page_{page_num+1}_table_{table_index+1}.csv"
                    )
...
def Ingest_phase_1(upload_folder, pqr_file):
    ...
    reporting_period_startdate, reporting_period_enddate, product_name, site_name = (
        ingest_pqr_file(tmp_pqr_pdf_path)
    )
    ...
    shutil.move(
        tmp_pqr_pdf_path, f"{input_folder_pqr_reporting_period_product_name}/{pqr_file}"
    )
...
def Ingest_phase_2(product_name, reporting_period, site_name, pqr_file_name):
    ...
    if (
        pinecone_document_noexists(
            pqr_file_name, reporting_period, product_name, site_name, INDEX_NAME
        )
        == 1
    ):
