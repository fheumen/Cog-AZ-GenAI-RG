...
from rga_format_text_hyperlink_utils import (
    highlight_run, clone_run_format, should_replace,
    highlight_subtext_in_hyperlink, handle_hyperlink_placeholders, replace_paragraph_text,
    apply_replacements_to_document, sitev_insert_text_in_report
)
...
def sitev_generation(report_id, file_version, user_id, session_id, timestamp, table_template_master, knowledge_base_name, model_id, template_fullname, bucket_name,  output_folder, source_file_names, source_file_types, kbname_max_size, name, upload_folder):
    ...
    try:
            ...
            sitev_insert_text_in_report(bucket_name, docx_s3_key, target_filename, data, highlight_color="green")
            if figures:
                sitev_insert_figure_in_report(bucket_name, figures, target_filename)
            ...
            if tables and (SOURCE_TYPE_TABLE_NAME in source_file_types):
                ...
                if deviation_change_values:
                    sitev_generate_report_from_template(bucket_name, target_filename, target_filename, deviation_change_values)
