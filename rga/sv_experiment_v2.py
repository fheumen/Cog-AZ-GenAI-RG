...
def highlight_run(run, color='green'):
...
def clone_run_format(source_run, target_run):
    ...
    """Copy formatting from source run to target run, including existing highlights."""
    ...
    # Preserve existing highlight color from source run
    ...
    try:
        ...
        if highlight_elem is not None:
            # Clone the highlight element to the target run
            target_rPr = target_run._element.get_or_add_rPr()

            # Remove any existing highlight first
            existing_highlight = target_rPr.find(qn('w:highlight'))

            if existing_highlight is not None:
                target_rPr.remove(existing_highlight)

            # Add the cloned highlight
            new_highlight = OxmlElement('w:highlight')
            new_highlight.set(qn('w:val'), highlight_elem.get(qn('w:val')))
            target_rPr.append(new_highlight)
...
def replace_paragraph_text(paragraph, replacements):
    ...
    """Replace text in paragraph, handling text split across multiple runs, preserving exact formatting."""
    ...
    # Build a map of character positions to their original run formatting
    char_to_run = []
    pos = 0
    
     # Step 1: Replace placeholders inside hyperlinks
    handle_hyperlink_placeholders(paragraph, replacements, 'green')
    ...
    # Process replacements and track which parts should be highlighted
    segments = []  # List of (text, source_run, is_highlighted)
    idx = 0

    while idx < len(full_text):
        matched = False

        # Try to match placeholders (longest first)
        for placeholder, replacement in sorted(replacements.items(), key=lambda x: -len(x[0])):
            if full_text.startswith(placeholder, idx):
                # Get the run formatting from where this placeholder starts
                source_run = char_to_run[idx] if idx < len(char_to_run) else paragraph.runs[0]
                segments.append((replacement, source_run, True))
                idx += len(placeholder)
                matched = True
                break
        ...
        if not matched:
            ...
            # No placeholder matched, keep the original character
            source_run = char_to_run[idx] if idx < len(char_to_run) else paragraph.runs[0]

            # Try to merge consecutive characters with same formatting
            if segments and segments[-1][1] == source_run and not segments[-1][2]:
                segments[-1] = (segments[-1][0] + full_text[idx], source_run, False)
            else:
                segments.append((full_text[idx], source_run, False))
            idx += 1 
    ...
    # Rebuild paragraph with proper formatting
    for text, source_run, is_highlighted in segments:
        new_run = paragraph.add_run(text)
        clone_run_format(source_run, new_run)
        if is_highlighted:
            highlight_run(new_run, 'green')
