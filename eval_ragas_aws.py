...
def evaluate_rag(question, ground_truth, product_name, reporting_period, chat_history: List[Dict[str, Any]] = []):
    trace = langfuse.trace(name = "rag")
    chat = ReportGeneration(embedding, opensearch_domain_endpoint, opensearch_index)
    question_with_context = question + f". Given that the product name is {product_name} and the reporting period is {reporting_period}"
    ...
    #print(answer)
    trace.span(
        name = "generation", input={'question': question, 'contexts': contexts}, output={'answer': answer["answer"]}
    )
    data = {
        'question': [question],
    ...
    result = evaluate(
        ...
        llm=chat.chat_model, embeddings=embeddings
    )
    for m in metrics:
        print(metrics)
...
for _, row in tqdm(test_dataset.iterrows()):
    answer = evaluate_rag(row['question'], row['ground_truth'], product_name, reporting_period, chat_history=chat_history)
    chat_history.append(
        (row['question'], answer)
    )