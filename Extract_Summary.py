...
def IS_StopExtraction(para, title_end="batches reviewed "):
    ...
    if len(re.findall(title_end.lower(), para.text.lower())) > 0:
        if (
            ...
            or ("Überschrift" in para.style.name)
            or ("titre" in para.style.name.lower())
            or ("title" in para.style.name.lower())
            or ("level" in para.style.name.lower())
            or ("gliederung" in para.style.name.lower())
...
def iSRS_write_in_log(res_code, filename, abs_filename=""):
    ...
    if res_code == 0:
        ...
        logging.error(
            ...
            + filename.ljust(105)
            + "  -->"
            + '  Failed: There is no "SUMMARY" in this Document'
        )
        ...
        logging.warning(
            ...
            + filename.ljust(105)
            + "  -->"
            + '   Failed: There is no "Conclusion" in "SUMMARY" in this Document'
        )
        logging.info("Extraction Job Ended...")
        ...
        logging.error(
            ...
            + filename.ljust(105)
            + "  -->"
            + "  Failed: Unable to open the file. Probably the file was not saved properly."
        )
        logging.info("Extraction Job Ended...")
        ...
        if check_of_succ_iSRS(abs_filename):
            ...
            if res_code == -5:
                logging.info(
                    ...
                    + filename.ljust(105)
                    + "  -->"
                    + '  Pass: Successful "SUMMARY" Extraction, but a Table in SUMMARY have not been formatted as expected'
                )
            else:
                logging.info(
                    "    "
                    + filename.ljust(105)
                    + "  -->"
                    + '  Pass: Successful "SUMMARY" Extraction'
                )
            logging.info("Extraction Job Ended...")
        else:
            ...
            logging.warning(
                ...
                + filename.ljust(105)
                + "  -->"
                + "  Warning: the extracted summary has probably more pages as expected"
            )
            logging.info("Extraction Job Ended...")
    else:
        LOG_FILENAME = define_log_file("warning", filename)
...
def Extract_Summary(
    parent,
    title_begin="summary and conclusion",
    title_end="batches reviewed",
    ...
    for child in parent_elm.iterchildren():
        ...
        if isinstance(child, CT_P):
            ...
            ##### Identification of the body of the text to be extracted till the key Word "Conclusion"
                ...
                conclusion_flag == 1
                and (
                    len(re.findall("Conclusion", para.text)) > 0
            ...
            ##### Identification of the Stop of extraction
                ...
                flag == 2
                and IS_StopExtraction(para, title_end)
                and conclusion_flag == 0
            ):
                print(para.text)
                flag = -1
    ...
    if flag == 3:
        flag = -2
    if conclusion_flag == 0 and flag == 2:
        flag = -2
